 ...
 * @ticket 38280
 */
class Tests_Term_wpGetTermCountForObjectType extends WP_UnitTestCase {

	/**
	 * Test when a taxonomy is invalid.
	 */
	public function test_wp_get_term_count_for_object_type_invalid_taxonomy() {
	 ...
	 */
	public function test_wp_get_term_count_for_object_type_object_not_in_taxonomy() {
		$actual = wp_get_term_object_count( 0, 'category', 'page' );

		$this->assertWPError( $actual );
		$this->assertSame( 'invalid_object_type', $actual->get_error_code() );
	}

	/**
	 * Test when an invalid term is passed.
	 */
	public function test_wp_get_term_count_for_object_type_invalid_term() {
	 ...
	 */
	public function test_wp_get_term_count_for_object_type_single_object_type() {
	 ...
	 */
	public function test_wp_get_term_count_for_object_type_non_post_type() {
		register_taxonomy(
			'wptests_tax',
			array(
				'user',
	 ...
	 */
	public function test_wp_get_term_count_for_object_type_multiple_object_types() {
	 ...
	 */
	public function test_wp_get_term_count_for_object_type_multiple_object_types_attachment() {
		register_taxonomy(
			'wptests_tax',
			array(
				'post',
	...
	/**
	 * Test when an object type has a count in meta cache.
	 */
	public function test_wp_get_term_object_count_from_meta() {
		$term_id = self::factory()->term->create( array( 'taxonomy' => 'category' ) );

		update_term_meta( $term_id, '_wp_object_count_post', 2 );
		$this->assertSame( 2, wp_get_term_object_count_from_meta( $term_id, 'post' ) );
	}

	/**
	 * Test when an object type has no meta cache, but has been counted (no relationships).
	 */
	public function test_wp_get_term_count_for_object_type_from_meta_empty() {
		$term_id = self::factory()->term->create( array( 'taxonomy' => 'category' ) );
		update_term_meta( $term_id, '_wp_counted_object_types', array( 'post', 'foo' ) );

		$this->assertSame( 0, wp_get_term_object_count_from_meta( $term_id, 'foo' ) );
	}

	/**
	 * Tests when an object type has not been counted.
	 */
	public function test_wp_get_term_count_for_object_type_from_meta_not_counted() {
		$term_id = self::factory()->term->create( array( 'taxonomy' => 'category' ) );
		update_term_meta( $term_id, '_wp_counted_object_types', array( 'post' ) );

		$this->assertFalse( wp_get_term_object_count_from_meta( $term_id, 'foo' ) );
	}
}
