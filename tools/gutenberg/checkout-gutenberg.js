...
const { readGutenbergConfig, isLocalRepoMode, isGutenbergRepoClone } = require( './gutenberg-utils' );
...
async function main( force ) {
	if ( ! isLocalRepoMode() ) {
		console.log( 'ℹ️  Skipping Gutenberg checkout: GUTENBERG_LOCAL_REPO is not set to true.' );
		console.log( '   Add GUTENBERG_LOCAL_REPO=true to your .env file to enable local repository mode.' );
		return;
	}

	if ( ! force && isGutenbergRepoClone() ) {
		console.log( 'ℹ️  The `gutenberg` directory is already a git clone.' );
		console.log( '   Use `npm run grunt gutenberg:checkout -- --force` to remove and re-clone.' );
		return;
	}
