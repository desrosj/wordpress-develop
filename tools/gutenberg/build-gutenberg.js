...
const { gutenbergDir, isGutenbergRepoClone } = require( './gutenberg-utils' );

// Paths
const rootDir = path.resolve( __dirname, '../..' );
const buildShaFile = path.join( gutenbergDir, '.gutenberg-build-sha' );
...
async function main() {
	if ( ! isGutenbergRepoClone() ) {
		console.log( 'ℹ️  Skipping Gutenberg build: gutenberg/ is not a git clone.' );
		console.log( '   This step only runs in local repository mode (GUTENBERG_LOCAL_REPO=true).' );
		return;
	}

	// Skip the build if HEAD has not changed since the last successful build.
	const headResult = spawnSync( 'git', [ 'rev-parse', 'HEAD' ], {
		cwd: gutenbergDir,
		encoding: 'utf8',
	} );
	const currentSha = headResult.status === 0 ? headResult.stdout.trim() : null;

	if ( currentSha && fs.existsSync( buildShaFile ) ) {
		const builtSha = fs.readFileSync( buildShaFile, 'utf8' ).trim();
		if ( builtSha === currentSha ) {
			console.log( `ℹ️  Gutenberg build is already up to date (${ currentSha.slice( 0, 12 ) }). Skipping.` );
			return;
		}
	}
	...
	try {
		...
		// Record the built SHA so subsequent runs can skip if HEAD hasn't changed.
		if ( currentSha ) {
			fs.writeFileSync( buildShaFile, currentSha );
		}
	} catch ( error ) {
