...
function wp_get_term_object_count( $term_id, $taxonomy, $object_type ) {
	...
	if ( 0 === $term->count ) {
		$count = 0;
	} elseif ( 1 >= count( $taxonomy_object->object_type ) ) {
		$count = $term->count;
	} else {
 ...
 * that are not post types must have their own separate mechanism for triggering
 * these counts.
 *
 * @since 5.0.0
 *
 ...
 * @access private
 * @since 2.3.0
 * @since 5.0.0 Store term counts on a per object type basis in meta.
 *
 * @global wpdb $wpdb WordPress database abstraction object.
 *
